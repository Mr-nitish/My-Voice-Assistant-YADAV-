...
def run():
    ...
    elif 'translate' in process or "translator" in process:
        process=process.replace("translate","")
        speak(" ok sir. plz enter the sentence below ")
        sentence=TextBlob(input("\n Enter the sentence = "))
        sentence1 = sentence.strip().split(';')
        result=sentence1.translate(to="ta")
        #result2=sentence.translate(to="hi")
        speak(result)
        print(result)
        #print(result2)
